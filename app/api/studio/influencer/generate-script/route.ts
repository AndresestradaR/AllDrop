...
const SCRIPT_SYSTEM = `You are an expert UGC video director for Latin American e-commerce. You create scene-by-scene scripts optimized for Veo 3.1 AI video generation.
...
OUTPUT FORMAT — You MUST respond with valid JSON only, no markdown, no backticks, no explanation:
{
  "scenes": [
    {
      "sceneNumber": 1,
      "action": "What the influencer physically does in this scene",
      "dialogue": "Exact Spanish dialogue the influencer says (informal Latin American Spanish)",
      "camera": "Camera angle and movement description",
      "sound": "Sound design notes (ambient, music mood, sound effects)",
      "veoPrompt": "English prompt for Veo 3.1 video generation, max 400 chars"
    }
  ]
}
...
2. "veoPrompt" MUST be in English (Veo renders better in English). Max 400 characters.
3. Every veoPrompt MUST include the influencer's visual descriptor for consistency across scenes.
...
export async function POST(request: Request) {
  try {
    ...
    if (!influencerId || !productDescription || !angle) {
      return NextResponse.json({ error: 'influencerId, productDescription y angle son requeridos' }, { status: 400 })
    }
...
VOICE GENDER: ${voiceGender} (use "${voiceInstruction}" in every veoPrompt)
...
Generate exactly ${sceneCount} scenes following the narrative arc for ${sceneCount} scenes. Each scene is 8 seconds. Make the dialogue feel natural and the veoPrompts visually compelling.`
    ...
    try {
      ...
      console.error('[GenerateScript] JSON parse error:', parseErr, 'Raw:', responseText.substring(0, 200))
    ...
    console.log(`[GenerateScript] Success, ${result.scenes.length} scenes`)
