  ...
  // ==================== PIPELINE TOOLS (execute entire flows directly — $0 Claude cost) ====================
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        product_details: { type: 'string', description: 'Resumen corto de detalles del producto (max 500 chars). Complementa a productContext.' },
        sales_angle: { type: 'string', description: 'Ángulo de venta seleccionado (hook + descripcion)' },
        ...
        colorPalette: {
          type: 'object',
          properties: {
            primary: { type: 'string', description: 'Color primario hex (ej: "#0F172A"). Fondo principal, textos dark.' },
            secondary: { type: 'string', description: 'Color secundario hex (ej: "#3B82F6"). Acentos, badges, botones.' },
            accent: { type: 'string', description: 'Color acento hex (ej: "#10B981"). Destacados, iconos, CTA.' },
          },
          description: 'OBLIGATORIO: Paleta de colores con hex codes. Pregunta al usuario o sugiere basado en el producto. TODOS los banners usan estos MISMOS colores para consistencia visual.',
        },
        productContext: {
          type: 'object',
          properties: {
            description: { type: 'string', description: 'Descripción vendedora del producto (2-3 párrafos)' },
            benefits: { type: 'string', description: 'Beneficios principales (5-7, separados por newline)' },
            problems: { type: 'string', description: 'Problemas que resuelve (3-5 dolores del cliente)' },
            ingredients: { type: 'string', description: 'Ingredientes/materiales/componentes (si aplica)' },
            differentiator: { type: 'string', description: 'Diferenciador vs competencia' },
          },
          description: 'OBLIGATORIO: Contexto detallado del producto. Usa la ficha creativa del paso L2.5. Estos campos se inyectan directamente en el prompt de generación de cada banner.',
        },
        typography: {
          type: 'object',
          properties: {
            headings: { type: 'string', description: 'Fuente para títulos. Default: "Montserrat: sans-serif geometrica, bold, moderna, impactante"' },
            subheadings: { type: 'string', description: 'Fuente para subtítulos. Default: "Open Sans: sans-serif humanista, limpia, legible"' },
            body: { type: 'string', description: 'Fuente para texto. Default: "Open Sans: sans-serif humanista, limpia, legible"' },
          },
