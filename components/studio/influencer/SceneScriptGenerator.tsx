...
export function SceneScriptGenerator({
  ...
  const handleGenerate = async () => {
    if (!selectedAngle) {
      toast.error(t.studio.influencer.scenes.selectAngleFirst)
      return
    }
    if (!productDescription.trim()) {
      toast.error(t.studio.influencer.scenes.describeProduct)
      return
    }
    ...
    try {
      ...
      setExpandedScene(0)
      toast.success(t.studio.influencer.scenes.scriptGenerated.replace('{count}', String(data.scenes.length)))
    } catch (error: any) {
  ...
  return (
    <div className="mt-3 p-4 rounded-2xl border border-teal-500/30 bg-gradient-to-b from-teal-500/5 to-transparent">
      <h3 className="text-sm font-bold text-teal-400 mb-3 flex items-center gap-2">
        ...
        {t.studio.influencer.scenes.title}
      </h3>
      ...
      <div className="mb-4">
        <label className="block text-xs font-semibold text-[#e5e5e5] uppercase tracking-wide mb-1 flex items-center gap-1.5">
          ...
          {t.studio.influencer.scenes.product}
        </label>
        {isLoadingProducts ? (
          <div className="flex items-center gap-2 px-3 py-2 bg-[#1a1a1a] border border-[#333] rounded-lg">
            <Loader2 className="w-3.5 h-3.5 animate-spin text-teal-400" />
            <span className="text-xs text-text-muted">{t.studio.influencer.scenes.loadingProducts}</span>
          </div>
        ) : products.length > 0 ? (
          <select
          ...
          <p className="text-[11px] text-text-muted px-3 py-2 bg-[#1a1a1a] border border-[#333] rounded-lg">
            {t.studio.influencer.scenes.noProducts}
          </p>
        )}
      </div>
      ...
      <div className="mb-4">
        ...
        >
          <span>{t.studio.influencer.scenes.selectAngle}</span>
            ...
            {selectedAngle && (
              <div className="mb-2 p-2 rounded-lg bg-teal-500/10 border border-teal-500/30">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {t.studio.influencer.scenes.change}
                  </button>
                </div>
      ...
      <div className="mb-4 space-y-3">
        <div>
          <label className="block text-xs font-semibold text-[#e5e5e5] uppercase tracking-wide mb-1">
            {t.studio.influencer.scenes.productDescriptionLabel} {productDescription && <span className="text-teal-400 text-[10px] font-normal ml-1">{t.studio.influencer.scenes.fromBannerGen}</span>}
          </label>
          <textarea
            value={productDescription}
            onChange={(e) => setProductDescription(e.target.value)}
            placeholder={t.studio.influencer.scenes.productPlaceholder}
            rows={3}
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-[10px] font-semibold text-text-muted uppercase tracking-wide mb-1">
              {t.studio.influencer.scenes.voice}
            </label>
            <select
              value={voiceGender}
              ...
              className="w-full px-3 py-2 bg-[#1a1a1a] border border-[#333] rounded-lg text-sm text-[#e5e5e5] focus:outline-none focus:ring-2 focus:ring-teal-500/50"
            >
              <option value="femenina">{t.studio.influencer.scenes.female}</option>
              <option value="masculina">{t.studio.influencer.scenes.male}</option>
            </select>
          </div>
          <div className="flex-1">
            <label className="block text-[10px] font-semibold text-text-muted uppercase tracking-wide mb-1">
              {t.studio.influencer.scenes.scenesLabel}: {numberOfScenes}
            </label>
            <input
              type="range"
              min={3}
      ...
      >
        {isGenerating ? (
          <>
            ...
            {t.studio.influencer.scenes.generatingScript}
          </>
        ) : (
          <>
            <Sparkles className="w-4 h-4" />
            {t.studio.influencer.scenes.generateScript.replace('{count}', String(numberOfScenes))}
          </>
        )}
      </button>
      ...
      {scenes.length > 0 && (
        <div className="mt-4 space-y-2">
          <h4 className="text-xs font-bold text-[#e5e5e5] uppercase tracking-wide">
            {t.studio.influencer.scenes.generatedScenes}
          </h4>
              ...
              >
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 space-y-2">
                    <div>
                      <span className="text-[9px] font-semibold text-text-muted uppercase">{t.studio.influencer.scenes.action}</span>
                      <p className="text-[11px] text-[#ccc]">{scene.action}</p>
                    </div>
                    <div>
                      <span className="text-[9px] font-semibold text-text-muted uppercase">{t.studio.influencer.scenes.dialogue}</span>
                      <p className="text-[11px] text-amber-400/80 italic">&quot;{scene.dialogue}&quot;</p>
                    </div>
                    <div className="flex gap-3">
                      <div className="flex-1">
                        <span className="text-[9px] font-semibold text-text-muted uppercase">{t.studio.influencer.scenes.camera}</span>
                        <p className="text-[10px] text-[#999]">{scene.camera}</p>
                      </div>
                      <div className="flex-1">
                        <span className="text-[9px] font-semibold text-text-muted uppercase">{t.studio.influencer.scenes.sound}</span>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        ...
                        >
                          ...
                          {t.studio.influencer.scenes.generateVideo}
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="mt-4 pt-4 border-t border-[#333]">
            <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wide mb-3 text-center">
              {t.studio.influencer.scenes.generateNScenes.replace('{count}', String(scenes.length))}
            </p>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <span className="font-bold">{t.studio.influencer.scenes.sequential}</span>
                <span className="text-[9px] text-teal-400/60 font-normal leading-tight text-center">
                  {t.studio.influencer.scenes.sequentialDesc}
                </span>
              </button>
              {/* Paralelo */}
              <button
              ...
              >
                ...
                <span className="font-bold">{t.studio.influencer.scenes.parallel}</span>
                <span className="text-[9px] text-emerald-400/60 font-normal leading-tight text-center">
                  {t.studio.influencer.scenes.parallelDesc}
                </span>
              </button>
            </div>
          </div>
